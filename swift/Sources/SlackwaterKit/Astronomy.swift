// Slackwater — MIT. Astronomical fundamentals.
// Faithful port of @slackwater/engine astronomy (src/astronomy/*).
// Mean longitudes and derived node angles in degrees; Slackwater is the golden oracle.
import Foundation
...
private enum Coeff {
    // Obliquity coefficients are each scaled by 0.01^index (Slackwater coefficients.ts).
...
/// Julian Date from a UTC instant (Slackwater JD()).
...
/// Astronomical state at an instant. Fields match Slackwater `astro()` keys (degrees).
struct Astro {
    let s, h, p, N, pp, ninety, omega, i: Double
    let I, xi, nu, nup, nupp: Double
    let ThMinusS, P: Double

    /// Slackwater-key → value map (for golden comparison against the reference).
...
/// Compute the astronomical state at a UTC instant (Slackwater `astro()`).
