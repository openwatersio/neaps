...
import { computeCompoundCorrection } from "../../src/node-corrections/compound.js";
...
describe("computeCompoundCorrection", () => {
  it("MS4 = M2 + S2: f=f_M2, u=u_M2", () => {
    const result = computeCompoundCorrection(constituents.MS4.members!, ihoStrategy.get, testAstro);
  ...
  it("MN4 = M2 + N2: f=f_M2², u=2*u_M2", () => {
    const result = computeCompoundCorrection(constituents.MN4.members!, ihoStrategy.get, testAstro);
    const m2 = ihoStrategy.get("M2", testAstro);
  ...
  it("2MK3 = 2×M2 − K1: f=f_M2²·f_K1, u=2u_M2−u_K1", () => {
    const result = computeCompoundCorrection(
      constituents["2MK3"].members!,
      ihoStrategy.get,
      testAstro,
    );
    const m2 = ihoStrategy.get("M2", testAstro);
  ...
  it("KO2 = K1 + O1: f=f_K1·f_O1, u=u_K1+u_O1", () => {
    const result = computeCompoundCorrection(constituents.KO2.members!, ihoStrategy.get, testAstro);
    const k1 = ihoStrategy.get("K1", testAstro);
  ...
  it("M4 (single-letter overtide): f=f_M2², u=2*u_M2", () => {
    const result = computeCompoundCorrection(constituents.M4.members!, ihoStrategy.get, testAstro);
    const m2 = ihoStrategy.get("M2", testAstro);
  ...
  it("ML4 = M2 + L2: uses L2 fundamental correction", () => {
    const result = computeCompoundCorrection(constituents.ML4.members!, ihoStrategy.get, testAstro);
    const m2 = ihoStrategy.get("M2", testAstro);
  ...
  it("all x-code constituents with members produce finite f > 0 and finite u", () => {
    for (const [name, c] of Object.entries(constituents)) {
      if (!c.members) continue;
      const result = computeCompoundCorrection(c.members, ihoStrategy.get, testAstro);
