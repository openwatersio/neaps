...
describe("compound nodal corrections", () => {
  it("MS4 = M2 + S2: f=f_M2, u=u_M2", () => {
    const result = constituents.MS4.correction(testAstro);
    const m2 = constituents.M2.correction(testAstro);
  ...
  it("MN4 = M2 + N2: f=f_M2², u=2*u_M2", () => {
    const result = constituents.MN4.correction(testAstro);
    const m2 = constituents.M2.correction(testAstro);
  ...
  it("2MK3 = 2×M2 − K1: f=f_M2²·f_K1, u=2u_M2−u_K1", () => {
    const result = constituents["2MK3"].correction(testAstro);
    const m2 = constituents.M2.correction(testAstro);
    const k1 = constituents.K1.correction(testAstro);
  ...
  it("KO2 = K1 + O1: f=f_K1·f_O1, u=u_K1+u_O1", () => {
    const result = constituents.KO2.correction(testAstro);
    const k1 = constituents.K1.correction(testAstro);
    const o1 = constituents.O1.correction(testAstro);
  ...
  it("2SM2 = 2×S2 − M2: f=f_M2, u=−u_M2", () => {
    const result = constituents["2SM2"].correction(testAstro);
    const m2 = constituents.M2.correction(testAstro);
  ...
  it("M4 (single-letter overtide): f=f_M2², u=2*u_M2", () => {
    const result = constituents.M4.correction(testAstro);
    const m2 = constituents.M2.correction(testAstro);
  ...
  it("4MN6 = 4×M2 − N2: f=f_M2⁵, u=3*u_M2", () => {
    const result = constituents["4MN6"].correction(testAstro);
    const m2 = constituents.M2.correction(testAstro);
  ...
  it("ML4 = M2 + L2: uses L2 fundamental correction", () => {
    const result = constituents.ML4.correction(testAstro);
    const m2 = constituents.M2.correction(testAstro);
    const l2 = constituents.L2.correction(testAstro);
  ...
  it("all constituents with members produce finite f > 0 and finite u", () => {
    for (const [name, c] of Object.entries(constituents)) {
      ...
      const result = c.correction(testAstro);
