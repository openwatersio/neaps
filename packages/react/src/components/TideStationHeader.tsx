...
export function TideStationHeader({ station, className }: TideStationHeaderProps) {
  ...
  return (
    <div className={`@container ${className ?? ""}`}>
      <h1 className="m-0 text-xl @lg:text-3xl font-semibold text-(--slackwater-text) @max-sm:truncate">
        {station.name}
      </h1>
      <span className="block mt-0.5 text-xs @sm:text-sm text-(--slackwater-text-muted) @max-sm:truncate">
